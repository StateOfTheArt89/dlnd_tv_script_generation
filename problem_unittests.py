    ...
    with tf.Graph().as_default():
        ...
        for x in range(batches.shape[2]):
            assert np.array_equal(batches[0,0,x], np.array(range(x * 35, x * 35 + batches.shape[3]))),\
                'Batches returned wrong contents. For example, input sequence {} in the first batch was {}. Expected value was {}'.format(x, batches[0,0,x], np.array(range(x * 35, x * 35 + batches.shape[3])))
            assert np.array_equal(batches[0,1,x], np.array(range(x * 35 + 1, x * 35 + 1 + batches.shape[3]))),\
                'Batches returned wrong contents. For example, target sequence {} in the first batch was {}. Expected value was {}'.format(x, batches[0,1,x], np.array(range(x * 35 + 1, x * 35 + 1 + batches.shape[3])))
